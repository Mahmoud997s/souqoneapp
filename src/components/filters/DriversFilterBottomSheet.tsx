...
interface DriversFilterBottomSheetProps {
  visible: boolean;
  onClose: () => void;
  initialFilters: DriverFilterState;
  onApplyFilters: (filters: DriverFilterState) => void;
}

const LICENSE_TYPES = [
  { id: 'LIGHT', label: 'خفيفة' },
  { id: 'HEAVY', label: 'ثقيلة' },
  { id: 'TRANSPORT', label: 'نقل' },
  { id: 'BUS', label: 'حافلات' },
  { id: 'MOTORCYCLE', label: 'دراجة' }
];

export function DriversFilterBottomSheet({
  visible,
  onClose,
  initialFilters,
  onApplyFilters,
}: DriversFilterBottomSheetProps) {
  const [filters, setFilters] = useState<DriverFilterState>({ ...initialFilters });
  ...
  const updateFilter = (key: keyof DriverFilterState, value: any) => {
  ...
  const availableWilayats = filters.location && WILAYAT_BY_GOVERNORATE[filters.location] ? WILAYAT_BY_GOVERNORATE[filters.location] : [];
            ...
            <TouchableWithoutFeedback>
              <View style={s.sheet}>
                ...
                <View style={s.header}>
                  ...
                  <Text style={s.title}>تصفية السائقين</Text>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    <View style={s.section}>
                      <TouchableOpacity
                        style={[s.toggleRow, filters.verifiedOnly && s.toggleRowActive]}
                        onPress={() => updateFilter('verifiedOnly', !filters.verifiedOnly)}
                        activeOpacity={0.8}
                      >
                        <Ionicons
                          name={filters.verifiedOnly ? 'shield-checkmark' : 'shield-outline'}
                          size={20}
                          color={filters.verifiedOnly ? Colors.primary : Colors.textMuted}
                        />
                        <Text style={[s.toggleText, filters.verifiedOnly && s.toggleTextActive]}>
                          عرض السائقين الموثقين فقط
                        </Text>
                      </TouchableOpacity>
                    </View>
                    ...
                    {/* الرخصة المطلوبة */}
                    <View style={s.section}>
                      <Text style={s.sectionTitle}>نوع الرخصة</Text>
                      <View style={s.row}>
                        {LICENSE_TYPES.map((e) => (
                          <TouchableOpacity
                            key={e.id}
...
const s = StyleSheet.create({
  ...
  toggleRow: {
    ...
    backgroundColor: Colors.surface, padding: Spacing.space3,
