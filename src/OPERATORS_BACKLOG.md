...
## [BACKLOG-3] معالجة أخطاء صامتة في رفع/حذف صور الباصات

**الخطورة**: 🔴 عالية (باگ صامت يخفي فشل حقيقي عن المستخدم)

**المشكلة**: `app/buses/new.tsx` بيستخدم `busesApi.addImages` و `busesApi.removeImage` جوه `try/catch` فارغة (`catch { /* ignore */ }`). لو فشل رفع صورة فعليًا، المستخدم بيشوف "تم بنجاح" رغم إن الصورة مترفعتش.

**اكتُشفت أثناء**: جلسة إصلاح صور المعدات بتاريخ 2026-08-20.

**الحل المطلوب**: معالجة أخطاء صريحة (رسالة للمستخدم لو فشلت صورة، بدل تجاهل صامت) — يحتاج جلسة منفصلة.
**لا تُنفَّذ الآن — قسم منفصل عن معدات المعدات.**
