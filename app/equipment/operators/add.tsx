...
export default function AddOperatorScreen() {
  ...
  const handleComplete = () => {
    ...
    createMutation.mutate(payload, {
      onSuccess: () => {
        alert('تم إضافة ملفك كمشغل بنجاح!')
        router.back()
      },
      onError: (err: any) => {
        const msg = err?.response?.data?.message || err?.message || 'حدث خطأ أثناء الإضافة'
        alert(msg)
      }
    })
  }
  ...
  const renderStep3 = () => (
    <View style={s.stepContent}>
      ...
      <View style={s.finishBox}>
        ...
        <Text style={s.finishTitle}>أنت جاهز للبدء!</Text>
        <Text style={s.finishDesc}>بالضغط على تأكيد سيتم مراجعة ملفك ونشره في قائمة المشغلين لتبدأ بتلقي العروض.</Text>
      </View>
    </View>
  )
    ...
    <View style={s.root}>
      ...
      >
        <View style={s.headerTop}>
          ...
          <Text style={s.headerTitle}>التسجيل كمشغل</Text>
          <View style={{ width: 24 }} />
        </View>
        {renderProgress()}
      </LinearGradient>
